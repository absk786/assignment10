...
function updateEmployeeRole(query, runApp) {
    updatemployee(query).then(async (data) => {
        console.log(query)
        console.log("update employee")
        const sql = "Update INTO employee (first_name,last_name,manager_id,role_id) VALUES (?,?,?,?);"
        await query(sql, [data.first_name,data.last_name,data.manager_id,data.role_id])
        console.table("employee was added")
        runApp()
    })
