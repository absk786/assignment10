...
function updateEmployeeData(query, runApp) {
    updateEmployee(query).then(async (data) => {
        const sql = "SELECT first_name FROM employee WHERE first_name =?;"
        await query(sql,data.selectEmployee)
        console.log("employee selected")
    })
    .then(setEmployeeInfo(query))
    // .then( async (data) => {
    //     const sql = "Update employee SET (first_name,last_name,manager_id,role_id) VALUES (?,?,?,?);"
    //     await query(sql, [data.first_name,data.last_name,data.manager_id,data.role_id])
    //     console.table("employee was added")
    //     runApp()
    // })
    ...
    return inquirer.prompt([
        {
            ...
            choices: async function () {
               ...
               console.table(displayEmployee)
            }
        }
    ]);
};

const setEmployeeInfo = async (query) => {
    console.log("setemployee started")
    return inquirer.prompt([
        {
            type: 'input',
            name: 'first_name',
